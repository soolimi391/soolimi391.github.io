...
<div class="chat-container">
  <div class="chat-box" id="chatBox">
    <div class="message bot">
      ...
      <div class="bubble">
        안녕하세요!<br>
        <strong style="color:#2b79ff;">로또 상담 챗봇</strong>입니다 😊<br>
        무엇이든 물어보세요! <strong style="color:#e60000;">로또 추천번호</strong>도 알려드려요!<br>
        예: '이번', '1234회차', '확률', '세금', '최대 당첨금'
      </div>
    </div>
  </div>
...
<script>
    ...
    function sendMessage() {
      ...
      setTimeout(() => {
        ...
        // ✅ 최신 회차 당첨번호
        if (msg.includes("번호") || msg.includes("이번") || msg.includes("당첨")) {
                ...
                fetch(`https://api.allorigins.win/get?url=${encodeURIComponent('https://www.dhlottery.co.kr/common.do?method=getLottoNumber&drwNo=' + latestRound)}`)
                  ...
                  .then(data => {
                    ...
                    if (lotto.returnValue === "success") {
                      ...
                      appendMessage(`🔔 <strong>${lotto.drwNo}회차 최신 당첨번호</strong><br>${nums} + 보너스 ${lotto.bnusNo}<br>1등 당첨금: <strong>${Number(lotto.firstWinamnt).toLocaleString()}원</strong>`, "bot");
                    }
                  });
              } else {
                appendMessage("최신 회차 정보를 가져오지 못했습니다.", "bot");
              }
            });
          return;
        }
        ...
        // ✅ 회차별 번호 + 당첨금
        if (msg.match(/(\d+)[\s]*회차/)) {
          const round = msg.match(/(\d+)[\s]*회차/)[1];
          fetch(`https://api.allorigins.win/get?url=${encodeURIComponent('https://www.dhlottery.co.kr/common.do?method=getLottoNumber&drwNo=' + round)}`)
            ...
            .then(data => {
              ...
              if (lotto.returnValue === "success") {
                const nums = [lotto.drwtNo1, lotto.drwtNo2, lotto.drwtNo3, lotto.drwtNo4, lotto.drwtNo5, lotto.drwtNo6].join(', ');
                appendMessage(`${round}회차 당첨번호는 <strong>${nums} + 보너스 ${lotto.bnusNo}</strong>입니다.<br>1등 당첨금: <strong>${Number(lotto.firstWinamnt).toLocaleString()}원</strong>`, "bot");
              } else {
                appendMessage(`${round}회차 정보를 찾을 수 없습니다.`, "bot");
              }
            })
            .catch(() => appendMessage("데이터를 불러오는 중 오류가 발생했습니다.", "bot"));
          return;
        }
  
        // ✅ Q&A 키워드 응답
        const qnaList = [
          { kw: ["확률"], msg: "로또 1등 당첨 확률은 약 1/8,145,060 입니다." },
          { kw: ["세금"], msg: "당첨금의 약 33%는 세금으로 공제됩니다." },
          { kw: ["판매"], msg: "판매 시간은 오전 6시~자정, 토요일은 오후 8시까지입니다." },
          { kw: ["가격", "얼마"], msg: "로또 1게임은 1,000원입니다." },
          { kw: ["최대", "당첨금"], msg: "역대 최고 당첨금은 2003년 4월 약 407억 원입니다!" },
          { kw: ["보너스"], msg: "보너스 번호는 2등을 결정하는 추가 번호입니다." },
          { kw: ["요일"], msg: "로또 추첨은 매주 토요일입니다." },
          { kw: ["문의"], msg: "문의 접수를 위해 이름과 전화번호를 입력해주세요." },
          { kw: ["후기"], msg: "최근 당첨자: 김*수님, 정*영님 - 축하드립니다! 🎉" }
        ];
        ...
        appendMessage("궁금한 것을 말씀해보세요! 예: '이번 당첨번호', '확률', '세금', '1234회차'", "bot");
      }, 500);
    }
  </script>
